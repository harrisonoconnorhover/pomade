...
Pause and resume preserve the same search. A 30-minute wait pauses for review; **Resume saved run** continues checking the original ID. Changed inputs require a new run. If the submit response is lost before its ID is saved, automatic resubmission stops; check Enrow's request history before deliberately starting a fresh run. Background, workbook and scheduled runs support these presets. Schedules now queue durable jobs and defer transfers/CRM writes until enrichment completes. Live provider access is still untested.

FullEnrich adds separate work-email, personal-email, mobile, and active-mobile-with-matched-owner presets. Connect in **Account → Connections**, or set `FULLENRICH_API_KEY` locally. Each lookup requests one contact type only. Work email uses first name, last name and company domain; personal email and mobile use LinkedIn. Email requires `DELIVERABLE`; catch-all and high-probability results are rejected. Both mobile presets require mobile line type and reject known inactive or mismatched-owner numbers. The stricter preset also requires `ACTIVE` and `CONFIRMED` for the same number; ownership checks are documented for US/Canada.

FullEnrich uses the same saved-request background runner with **at least five minutes between result checks**, as required by its polling guidance. No public callback URL is needed. Pending results hold the waterfall; completed misses allow fallback. Only the saved batch and its matching contact are accepted. A 30-minute wait or result error stops for review; Resume checks the saved request. Unknown submission outcomes require checking FullEnrich history before starting a new run. Background, workbook and scheduled jobs support this. Submission cost stays unknown until FullEnrich reports it with the results; cumulative result credits are counted by their newly observed amount, so repeated checks do not add the same total again. Synthetic tests do not establish live access or billing accuracy.
...
### Find, verify and measure the waterfall

In **Provider waterfall**, each finder has an optional **Verify this result before accepting it** choice. The candidate goes straight to an existing Hunter, LeadMagic, Findymail, ZeroBounce, BounceBan or Enrow email verifier, or Trestle phone validator. No intermediate verification column is needed. Rejected results fall back to the next finder; pending results hold the chain; technical failures stop unless the existing continue-on-error option permits continuation. Verification must refer to the same candidate. Trestle validity does not prove mobile type, ownership or reachability.

One waterfall still has up to four finders. Optional verifiers count toward request budgets, so four finders with four verifiers allow up to eight provider submissions per row. Save a workflow without keys; connect each required provider before running. Asynchronous verification automatically uses background execution. Saved templates retain the verifier choice.

**Run history → Provider performance** summarizes up to 100 recent run receipts for the current sheet: finder lookups, verification checks, accepted matches, extra matches from later finders, waiting/errors, elapsed result time and observed credits. Polls and reused steps do not count as extra lookups. Elapsed time includes provider waiting. Unreported cost stays unknown; vendor credits are not interchangeable currencies or a dollar estimate. Older receipts without operation identity are excluded from these match comparisons. This is observed usage, not an independently measured provider coverage benchmark.

### Saved scheduled runs

Schedules use the same persisted background jobs as manual runs, with up to 100 rows, 50 provider submissions per run and 10 per row, including optional verifiers and possible source additions. A completed source refresh is reused while the occurrence waits. Only after enrichment completes do captured CRM writes and row transfers begin. Each recurring occurrence gets a fresh execution ID.
