...
function fixture(presetId = 'enrow-email', surrounding = false) {
  ...
  const connections = [
    ...configuredHttpConnections({ ENROW_API_KEY: 'synthetic-private-key' }),
    ...['first', 'last'].map((id) => ({
      id,
      label: id,
...
describe('Enrow durable background lookups', () => {
  ...
  it.each(['lost', 'missing-id', 'server-error'])(
    ...
    async (kind) => {
      ...
      expect((await f.run()).receipt.error).toContain('no search ID was saved');
      expect((await f.run()).receipt.error).toContain('no search ID was saved');
