...
1. **Find → verify → fallback in one action.** Each finder can pass its candidate to a supported email verifier or phone validator without an intermediate column. Rejection advances; pending waits; technical errors retain the existing stop/continue rules.
