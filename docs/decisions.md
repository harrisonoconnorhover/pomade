...
### 2026-09-07 — Complete phone navigation and the private release
- At widths up to 500px, the permanent add-column rail is a compact 48px control and the first data column scrolls with the others. Wider layouts retain the full rail and pinned first column. This changes presentation only, never saved widths, values or recipes.
- Lookup table discovery distinguishes loading, empty results and failures; Retry tables recovers without leaving the builder. The source preview remains unavailable until the current table list and source load succeed.
- Final local and hosted browser checks passed nine flows with zero saved-data writes and zero page errors, including right-click discovery, research drafts, mobile lookup and horizontal grid scrolling. The final release preserved all original sheets and seven hosted connections. Owner-only hosted version 32 uses runtime source `41e0d54`; local runtime is the same source. No GitHub push occurred.
- Rebuilt the ignored local font cache after finding stale temporary-directory URLs. Cached font URLs now resolve under `/_next/static/_vinext_fonts/`; no layout or dependency change was needed. Final workbook entry measured 513,677 bytes / 146,712 gzip bytes locally, a 25.3% gzip reduction from the earlier 196,409-byte entry. This is an artifact measurement, not a load-time benchmark.


### 2026-09-07 — Extended night shift: edit recipes and navigate results
- Research column settings use local drafts for prompt, provider, model and effort; Cancel discards changes. Saving preserves stable IDs, output fields, input mappings and all existing results. App-default inheritance stays distinct from explicit Codex defaults. Changed research settings pause an enabled schedule; renaming alone does not.
- Existing provider waterfalls can reorder their saved steps and choose whether to continue after an error. Request definitions, verifiers, template bindings and remapped output IDs remain intact. A paused/failed job using that column must finish or be cancelled before changing its order, because resumable waterfall progress depends on the prior configuration. No provider call is made while configuring.
- Columns search now opens settings directly without revealing hidden columns. Keyboard selection keeps the inspector in sync, and source-row links scroll/select after the grid viewport is measured. Run receipts paginate all action details, 100 per page. Filter now exposes explicit status and saved-view choices instead of cycling between two states.
- Verification: 58 focused tests passed; desktop/390px browser checks covered draft/cancel/save/reopen, model inheritance, full receipts, hidden-column access, filters and offscreen row links. A real local QA waterfall flag saved, survived reload and was restored with all other sheet contents unchanged. Missing model lists now expose errors and Refresh models inside the editor.
...
### 2026-09-09 — Replace the new Personal opener shortcut with sourced research
- New Personal opener columns use the existing structured web-research engine and reusable template mapping. The editable prompt requests a concise factual opener, supporting quote, source URL and assessment; insufficient evidence should leave the opener blank. Existing citation and Review handling remain authoritative; no new provider or output parser was introduced.
- Adding the recipe creates four blank outputs with auto-run disabled. Provider access is checked at execution. Saved legacy `write-opener` columns keep their generic local behavior and values; their settings explain the difference and offer deliberate creation of separate researched columns.
- Validation uses a synthetic provider response through the real client/parser/pipeline and an intercepted browser workbook. It proves mapping, citation storage, missing-evidence handling and non-execution during configuration, not live research accuracy. This increment is local only; no hosted deployment, provider charge or CRM write.
